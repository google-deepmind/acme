...
from acme import core, environment_loop, specs, types
from acme.agents.jax import actor_core as actor_core_lib
from acme.agents.jax import actors, bc
...
def make_network(spec: specs.EnvironmentSpec) -> bc.BCNetworks:
    ...
    def log_prob(
        logits: networks_lib.Params, actions: networks_lib.Action
...
def make_demonstrations(env: dm_env.Environment, batch_size: int) -> tf.data.Dataset:
    ...
    transition = functools.partial(
        _n_step_transition_from_episode, n_step=1, additional_discount=1.0
    )
