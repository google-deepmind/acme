...
_NUM_STEPS = flags.DEFINE_integer(
    "num_steps", 10000, "Number of env steps to run training for."
)
_EVAL_EVERY = flags.DEFINE_integer("eval_every", 1000, "How often to run evaluation.")
_ENV_NAME = flags.DEFINE_string(
    "env_name", "MultiGrid-Empty-5x5-v0", "What environment to run."
)
...
def _make_network_factory(
    ...
    def environment_factory(
        ...
        return decentralized.network_factory(
            environment_spec, agent_types, helpers.init_default_multigrid_network
        )
...
def build_experiment_config() -> experiments.ExperimentConfig[
    ...
    config_overrides = {k: ppo_configs for k, v in agent_types.items() if v == "ppo"}

    configs = decentralized.default_config_factory(
        agent_types, _BATCH_SIZE.value, config_overrides
    )
