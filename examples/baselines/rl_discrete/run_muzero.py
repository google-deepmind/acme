...
NUM_ACTORS_PER_NODE = flags.DEFINE_integer(
    "num_actors_per_node", 2, "Number of colocated actors",
)
...
def build_experiment_config() -> experiments.ExperimentConfig:
    ...
    def network_factory(spec: specs.EnvironmentSpec,) -> muzero.MzNetworks:
        return muzero.make_network(spec, stack_size=muzero_config.stack_size,)
    ...
    muzero_builder = muzero.MzBuilder(  # pytype: disable=wrong-arg-types  # jax-ndarray
        muzero_config, extra_spec,
    )

    checkpointing_config = experiments.CheckpointingConfig(
        replay_checkpointing_time_delta_minutes=20, time_delta_minutes=1,
    )
...
def main(_):
    ...
    inference_server_config = inference_server_lib.InferenceServerConfig(
        batch_size=64, update_period=400, timeout=datetime.timedelta(seconds=1,),
    )
