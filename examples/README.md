...
-   [DQN](atari/run_dqn.py): a "classic" benchmark agent for Atari; and
-   [R2D2](atari/run_r2d2.py): a more recent Q-learning variant which includes
    recurrence.
...
-   [BC](offline/run_bc.py): a behaviour cloning agent.
-   [BC (JAX)](offline/run_bc_jax.py): a behaviour cloning agent (implemented
    in jax).
-   [BCQ](offline/run_bcq.py): an implementation of BCQ.

Similarly we also include so-called "from demonstration" agents which mix
offline and online data:

-   [DQfD](offline/run_dqfd.py): the DQfD agent running on hard-exploration
...
-   [DQN](bsuite/run_dqn.py): an off-policy DQN examples;
-   [Impala](bsuite/run_impala.py): an on-policy Impala agent; and
-   [MCTS](bsuite/run_mcts.py): a model-based agent running on the
