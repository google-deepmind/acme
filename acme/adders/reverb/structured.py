...
class StructuredAdder(adders_base.Adder):
  ...
  def __init__(self,
    ...
    """
    step_spec = create_step_spec(environment_spec, extras_spec)
  ...
  def reset(self, timeout_ms: Optional[int] = None):
    ...
    if self._writer is not None:
      # Flush all pending items but keep the buffers in case trajectories span
      # multiple episodes.
      self._writer.end_episode(clear_buffers=False, timeout_ms=timeout_ms)
...
def create_sequence_config(
  ...
  def _last_n(n: int):
    ...
    return Trajectory(*tree.map_structure(lambda x: x[-n:], ref_step))

  # When trajectories are allowed to span episode boundaries then our job
  # is very simple. Simply write an item every `period` steps spanning the last
  # `sequence_length` steps.
  if end_of_episode_behavior == EndBehavior.CONTINUE:
    return [
        sw.create_config(
            pattern=_last_n(sequence_length),
            table=table,
            conditions=[
                sw.Condition.steps_since_applied() >= period,
            ]),
    ]
  ...
  if end_of_episode_behavior == EndBehavior.WRITE:
    ...
    for x in range(period):
      ...
      config = sw.create_config(
          pattern=_last_n(unpadded_length),
          table=table,
          conditions=[
              sw.Condition.is_end_episode(),
