...
class DQNLearner(acme.Learner, tf2_savers.TFSaveable):
  ...
  def _step(self) -> Dict[str, tf.Tensor]:
    ...
    legal_actions_t = e_t.legal_actions
    terminals_t = e_t.terminals
    ...
    with tf.GradientTape() as tape:
      ...
      # TODO Here we apply the legal actions mask.
      illegal_actions = 1 - legal_actions_t
      illegal_logits = illegal_actions * ILLEGAL_ACTION_LOGITS_PENALTY
      q_t_value = tf.math.add(tf.stop_gradient(q_t_value), illegal_logits)
      q_t_selector = tf.math.add(tf.stop_gradient(q_t_selector), illegal_logits)

      # The rewards and discounts have to have the same type as network values.
      r_t = tf.cast(r_t, q_tm1.dtype)
      # TODO Remove reward clipping in OpenSpiel DQN?
