...
def make_objectives() -> Sequence[Union[mompo.Objective, mompo.Reward]]:
  """Define the multiple objectives for the policy to learn."""
  task_reward = mompo.Reward(
      name='task',
      reward_fn=task_reward_fn)
  action_norm = mompo.Objective(
      name='action_norm_q',
      objective_fn=compute_action_norm)
...
class MOMPOTest(parameterized.TestCase):
  ...
  def test_mompo(self, distributional_critic):
    ...
    num_critic_heads = len([x for x in objectives if hasattr(x, 'reward_fn')])
