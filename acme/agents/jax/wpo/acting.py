...
"""Acting logic for the WPO agent."""

from typing import Mapping, NamedTuple, Tuple, Union

from acme.agents.jax import actor_core as actor_core_lib
from acme.agents.jax.wpo import networks
...
def make_actor_core(wpo_networks: networks.WPONetworks,
                    stochastic: bool = True,
                    store_core_state: bool = False,
                    store_log_prob: bool = True) -> actor_core_lib.ActorCore:
  """Returns a WPO ActorCore from the WPONetworks."""
  ...
  def init(key: jax_types.PRNGKey) -> ActorState:
    ...
    params_initial_state = wpo_networks.torso.initial_state_fn_init(
        key, batch_size)
    core_state = wpo_networks.torso.initial_state_fn(params_initial_state,
                                                     batch_size)
    return ActorState(
        key=next_key,
  ...
  def select_action(params: networks.WPONetworkParams,
    ...
    policy = wpo_networks.policy_head_apply(params, embeddings)
